...
桌面版新增「🤖 自动签到」卡片：登录后开启，到点自动完成当天查寝，无需命令行。

1. 登录成功后，在自动签到卡片设置：
   - **触发时刻**：如 `22:00`，多个用逗号分隔（早/晚查寝）
   - **重查间隔 / 最大重查**：到点没任务时窗口内重查（默认 5 分钟 × 12 次 = 1 小时）
   - **照片池目录**：选一个放照片的文件夹，任务要拍照时随机选一张上传
   - **记住密码**：勾选后凭据存本机 `auto_config.json`，会话过期自动重登
2. 点「▶ 开启自动签到」，保持程序运行（可最小化到任务栏，关窗即停）
3. 到点自动签到，结果写入 GUI 日志区 + `auto_sign.log` 文件；如需人不在场也收到结果，把 pushplus 的 token 填入 `auto_config.json` 的 `pushplusToken` 字段（微信推送，留空关闭）

> 说明：电脑睡眠错过触发点会在唤醒后立即补签；自动签到与手动签到互斥，不会并发提交。
...
├── app.py              # tkinter 桌面版（含自动签到）
├── main.py             # 命令行版
├── config.yml          # 配置（学校、校区、密钥）
├── auto_config.json    # 自动签到设置与凭据（自动生成，勿提交）
├── auto_sign.log       # 自动签到日志（自动生成）
├── photos/             # 自动签到照片池（自行创建）
